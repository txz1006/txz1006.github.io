...
完校的APP端和后台端的数据加密传输是这样的：RSA+DES

APP端携带RSA公钥请求完校后台秘钥交换接口，后台收到公钥后生成DES对称秘钥和sessionId，同时根据这些秘钥信息，创建session对象存入redis中，然后将sessionId和DES秘钥使用入参RSA公钥加密后返回给APP端，APP端收到信息后用私钥解密出DES秘钥，之后的数据请求都使用这个动态DES秘钥来加解密。
...
- 创建方言格式对象和session对象(hibernateTemplet)
  ...
  public class AbstractHibernateTemplet {
      private SessionFactoryProxy sessionFactory;
      ...
      public SessionFactoryProxy getSessionFactory() {
          return sessionFactory;
      }
  
      public void setSessionFactory(SessionFactoryProxy sessionFactory) {
...
```
	<bean id="dataSource_iface" class="net.newcapec.v3.extend.jdbc.datasource.ProxoolDriverManagerDataSource">
		<property name="driverClassName" value="org.logicalcobwebs.proxool.ProxoolDriver"/>
		<property name="xmlFile" value="classpath:proxool_iface.xml"/>
		<property name="url" value="proxool.proxoolPool_iface"/>
	</bean>
```
...
```xml
    <bean id="iface_sessionFactory"
          class="net.newcapec.bsacs.utils.CampusIfaceAnnotationSessionFactoryBean">
        <property name="packagesToScan">
            <list>
                <value>net.newcapec.campus.ifaceserver.entity</value>
                <value>net.newcapec.campus.oauth2.entity</value>
            </list>
        </property>
        <property name="dataSource" ref="dataSource_iface"/>
        <property name="propertieLocation" value="classpath:hibernate.config.properties"/>
    </bean>
    <bean id="transactionManager_iface" class="org.springframework.orm.hibernate3.HibernateTransactionManager"
          p:sessionFactory-ref="iface_sessionFactory" p:dataSource-ref="dataSource_iface"/>
...
```xml
    <bean id="iface_dataSourceAdvice" class="net.newcapec.v3.extend.jdbc.aop.DataSourceAdvice"/>      
    <bean id="transactionManager_iface" class="org.springframework.orm.hibernate3.HibernateTransactionManager"
          p:sessionFactory-ref="iface_sessionFactory" p:dataSource-ref="dataSource_iface"/>  
	//manager层事务代理对象
    <bean id="transactionProxy_iface" class="org.springframework.transaction.interceptor.TransactionProxyFactoryBean"
          abstract="true">
        <property name="transactionManager" ref="transactionManager_iface"/>
        <property name="preInterceptors">
            <list>
                <ref bean="iface_dataSourceAdvice"/>
            </list>
        </property>
        <property name="transactionAttributes">
            <props>
                ...
                <prop key="fast*">PROPAGATION_SUPPORTS</prop>
    ...
    <bean id="iface_dialectManager" class="net.newcapec.v3.plugin.hibernate.DialectManagerImpl"
          p:sessionFactoryProxy-ref="iface_sessionFactory"/>
    <!-- hihbernate DAO配置模板 -->
    <bean id="hibernateTemplet_iface" abstract="true"
          class="net.newcapec.bsacs.utils.AbstractHibernateTemplet"
          p:sessionFactory-ref="iface_sessionFactory"
          p:dialectManager-ref="iface_dialectManager"/>
```
...
```xml
<!-- hihbernate DAO配置模板 -->
<bean id="appDao" class="net.newcapec.campus.ifaceserver.dao.impl.AppDaoImpl" parent="hibernateTemplet_iface"/>
<!-- manager层事务使用 -->
<bean id="customerManager" parent="transactionProxy_iface">
   		<property name="target">
   			<bean class="net.newcapec.campus.ifaceserver.manager.impl.CustomerManagerImpl" p:dao-ref="customerDao">
				<property name="appKeyFlagCache" ref="appKeyFlagCache"/>
			</bean>
   		</property>
</bean>
```
