...
- [x] 处理内蒙古师范大学缴费平台对接开放平台问题（新应用对接，处理中）
- [x] 处理三亚学校对接问题（学校安全演练，拉黑完校ip导致前置异常）
- [x] 长沙唯上智慧校园应用对接开放平台（处理中）

---------------

- [ ] 黔农云支付宿主对接（银行业务整改，改为三方对接方式，待处理）
- [ ] 运营平台广告个人屏蔽改造（05.23-27周开始开发，此前需要确定设计方案）
- [ ] 新增树维改造宿主，本地缓存公钥；改造B模块跳转模式，light直接回调到三方地址(移交龚一凡做)
- [ ] 开放平台处理用户会话失效不覆盖问题
...
- [x] 山西大学快捷支付页面，用户信息数据接口开发(标卡(缓存)-》开放平台-》完校后台-》一卡通)(待上线)

- [x] 日志平台改kafka连接（待上线，待区分线程池，待检查strus2支持问题）

- [x] 黔农云支付宿主对接（调测中）

- [x] 运营平台广告个人屏蔽改造（5%）

- [x] 处理六盘水师范学校应用不通问题（学校回调地址端口不通）
...
- [x] 黔农云支付宿主对接（待上线）
- [x] 运营平台广告个人屏蔽改造（5%）
- [x] 处理中山三鑫学校部分用户访问在线报修报错问题（该用户钉钉信息不完整，没有学工号信息）
...
- [x] 山西大学快捷支付页面，用户信息数据接口开发(标卡(缓存)-》开放平台-》完校后台-》一卡通)(待上线)
- [x] 运营平台广告个人屏蔽改造（45%）
- [x] 协助处理江西电力职业技术学院支付失败问题（宿主未配置正确）
- [x] 阜阳职业技术学校外嵌不通问题（学校接口问题，获取不到用户信息了）
...
- [x] 山西大学快捷支付页面，用户信息数据接口开发(标卡(缓存)-》开放平台-》完校后台-》一卡通)(待上线)
- [x] 运营平台广告个人屏蔽改造（75%）
- [x] 阜阳职业技术学校外嵌不通问题（学校接口问题，获取不到用户信息了，已处理）
- [x] 处理新乡学院超级APP嵌入健康系统问题（树维系统对接配置不对）
- [x] 配合完成日志系统升级测试工作
- [x] 处理绵阳师范学院标卡对接问题（处理中）
...
- [x] 中山三鑫学校在线报修部分用户报错问题（钉钉授权信息不完整导致）
...
- [x] 运营平台广告个人屏蔽改造（已开发完成，公测中）

- [x] 处理安溪梧桐中学中行B2C对接问题（处理中）

- [x] 处理科士达充电桩技术对接问题（处理中）

- [x] 完校客户端JSapi权限限制功能开发（5%，待开始）
...
- [x] 运营平台广告个人屏蔽改造（公测中）
- [x] 完校客户端JSapi权限限制功能开发（10%，待开始）
- [x] 完校后台日志信息发送kafka兼容改造（30%，大数据提供的cp_ecard_area_click_log、alipay_applet_clicklog已上两个表没有在完校后台找到，暂不处理）
- [x] 处理安溪梧桐中学中行B2C对接问题（处理中，待提供具体修改需求）
...
- [x] 运营平台广告个人屏蔽改造（公测中）
- [x] 完校客户端JSapi权限限制功能开发（10%，待开始）
- [x] 完校后台日志信息发送kafka兼容改造（60%）
- [x] 处理海大学校树维一卡通对接人脸应用问题（处理中）
...
- [x] 黔农云支付宿主对接（上线验证中）
- [x] 运营平台广告个人屏蔽改造（公测中）
- [x] 完校客户端JSapi权限限制功能开发（10%，待开始，暂定下周协定）
- [x] 完校后台日志信息发送kafka兼容改造（60%）
- [x] 处理海大学校树维一卡通对接人脸应用问题（处理中）
- [x] 协助处理云南林业学校华为link应用在IOS调不起农行掌银问题（处理中）
- [x] 协助处理湖南省联通（长沙唯上）充值接口对接问题（处理中）
- [x] 协助排查支付宝小程序调用开放平台认证接口报错问题（处理中）
...
- [x] 完校客户端JSapi权限限制功能开发（10%，待开始，暂定下周协定）
- [x] 完校后台日志信息发送kafka兼容改造（60%）
- [x] 处理呼和浩特民族学院访客应用对接问题（处理中，支付缺失openid，需要走ewpp获取openid流程，待开发）
- [x] 协助李盼盼配置蓝牙水控应用测试（处理中）
- [x] 处理河大外嵌洗澡应用异常（洗澡应用后台报错导致）
- [x] 处理甘肃建院学工对接开放平台问题（处理中）
- [x] 处理中浦院在线报修问题反馈（处理中）
- [x] 协助陈威配置洛阳职业技术学学院-工行快捷支付测试（处理中）
- [x] 处理未来学校标准对接问题（新学校对接）

2022.06.09

- [x] 运营平台广告个人屏蔽改造（公测中）
- [x] 完校客户端JSapi权限限制功能开发（10%，待开始，暂定下周协定）
- [x] 完校后台日志信息发送kafka兼容改造（公测中）
- [x] 处理呼和浩特民族学院访客应用对接问题（处理中，支付缺失openid，需要走ewpp获取openid流程，待开发）
- [x] 协助李盼盼配置蓝牙水控应用测试（处理中，去除建行e码通宿主跳转freemaker页面逻辑，待开发）
- [x] 迪科宿主支持微信支付改造（公测中）
- [x] 减少对接地址长度，增加短码映射宿主名称功能（待处理）
- [x] 客户端增加公有应用转私有应用按钮功能（待处理）
- [x] 短信平台处理Struts、fastjson漏洞问题以及升级事宜（待处理）
- [x] 处理中南财经政法大学对接问题（处理中）
...
- [x] 运营平台广告个人屏蔽改造（公测中）
- [x] 完校后台日志信息发送kafka兼容改造（公测中）
- [x] 完校客户端JSapi权限限制功能开发（10%，待开会确定）
- [x] 迪科宿主支持微信支付改造（公测中）
- [x] 减少对接地址长度，增加短码映射标准宿主名称功能（公测中）
- [x] 蓝牙水控小程序兼容外嵌处理（公测中）
- [x] 安溪中学开放平台绑卡页面增加提示语配置（待处理）
- [x] 短信平台处理Struts、fastjson漏洞问题以及升级事宜（待处理）
...
- [x] 蓝牙水控小程序兼容外嵌处理（公测中）
- [x] 安溪中学开放平台绑卡页面增加提示语配置（待处理）
- [x] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [x] 开放平台客户端增加公有应用转私有应用按钮功能（待处理）
- [x] 短信平台处理Struts、fastjson漏洞问题以及升级事宜（待处理，搭建公测环境）
...
- [x] 开放平台客户端增加公有应用转私有应用按钮功能（待处理）
- [x] 短信平台处理Struts、fastjson漏洞问题以及升级事宜（待处理，搭建公测环境）
- [x] 开放平台对接六盘水师范集微助教应用（新应用对接）
- [x] 处理开放平台日志写入到不同日志表的问题（待排查）
- [x] 科士达三方付对接方案开放平台接口开发（待确定）
...
- [x] 减少对接地址长度，增加短码映射标准宿主名称功能（公测中）
- [x] 蓝牙水控小程序兼容外嵌处理（公测中）
- [x] 安溪中学开放平台绑卡页面增加提示语配置（公测中）

- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [ ] 开放平台客户端增加公有应用转私有应用按钮功能（待处理）
- [ ] 短信平台处理Struts、fastjson漏洞问题以及升级事宜（待处理，搭建公测环境）
- [ ] 未来学校外嵌H5到第三方APP对接问题处理（处理中）
- [ ] 处理六盘水师范对接开放平台问题（处理中）
- [ ] 处理广告屏蔽测试、B模块升级内容测试产生的问题（处理中）

科士达三方付对接方案开放平台接口开发（待确定）
...
- [x] 运营平台广告个人屏蔽改造（待升级）
- [x] 完校后台日志信息发送kafka兼容改造（待升级）
- [x] 迪科宿主支持微信支付改造（已升级）
- [x] 减少对接地址长度，增加短码映射标准宿主名称功能（已升级）
- [x] 蓝牙水控小程序兼容外嵌处理（已升级）
- [x] 安溪中学开放平台绑卡页面增加提示语配置（待升级）

- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [ ] 开放平台客户端增加公有应用转私有应用按钮功能（待升级）
- [ ] 短信平台处理Struts、fastjson漏洞问题以及升级事宜（待测试，搭建公测环境）
- [ ] 未来学校外嵌H5到第三方APP对接问题处理（处理中）
- [ ] 处理六盘水师范对接开放平台问题（处理中）
- [ ] 处理广告屏蔽测试、B模块升级内容测试产生的问题（已完成）
- [ ] 南京外国语标卡H5对接沐坤问题处理（沐坤传了一个token字段，传到标卡时）
- [ ] 协助排查部分用户南京工业大学标卡校园码没有设备码报错问题（标卡升级后访问路径有变化导致）
- [ ] 整理开放平台缴费相关接口文档

2022.06.23

- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [x] 开放平台客户端增加公有应用转私有应用按钮功能（待升级）
- [x] 短信平台处理Struts、fastjson漏洞问题以及升级事宜（待测试，搭建公测环境）
- [ ] 未来学校外嵌H5到第三方APP对接问题处理（处理中）
- [ ] 处理六盘水师范对接开放平台问题（处理中）
- [ ] 处理广告屏蔽测试、B模块升级内容测试产生的问题（已完成）
- [ ] 南京外国语标卡H5对接沐坤问题处理（沐坤传了一个token字段，传到标卡时）
- [ ] 协助排查部分用户南京工业大学标卡校园码没有设备码报错问题（标卡升级后访问路径有变化导致）
- [ ] 协助处理河南农业大学微信水电应用支付失败问题（已处理）
...
- [ ] 运营平台广告个人屏蔽改造（待升级）
- [ ] 完校后台日志信息发送kafka兼容改造（待升级）
- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [x] 开放平台客户端增加公有应用转私有应用按钮功能（已升级）
- [x] 协调处理中山三鑫学校钉钉应用获取用户信息缺少字段问题（处理中）
- [x] 安溪中学开放平台绑卡页面增加提示语配置（已升级）
- [x] 整理科士达缴费一卡通接口文档（已完成）
- [x] 开放平台科士达缴费一卡通接口开发（10%）
...
- [ ] 完校后台日志信息发送kafka兼容改造（待升级）
- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [x] 协调处理中山三鑫学校钉钉应用获取用户信息缺少字段问题（已处理）
- [ ] 开放平台科士达缴费一卡通接口开发（10%，待双方确定文档后开发）
- [ ] 处理浙师大保卫处访客应用CAS对接问题（处理中）
- [ ] cassso宿主增加ewpp获取支付信息开发（待处理）
...
- [ ] 完校后台日志信息发送kafka兼容改造（待升级）
- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [ ] cassso宿主增加ewpp获取支付信息开发（50%）
- [ ] 拦截宿主请求请求参数，动态修改开放平台回调url结构（待开发）
- [ ] 中行宿主改造（待确定）
- [ ] 
- [ ] 开放平台科士达缴费一卡通接口开发（待双方确定文档后开发）
- [ ] 开放平台非授权访问增加log文档日志（待增加）
...
开放平台非授权应用缓存和数据库数据不一致导致原因：十有八九的确是B模块启动导致的，7月1号刚好有几次重启和问题反馈时间对应上了。

处理办法如下：

1.缓存key修改，appKeyFlagCache:appMap改为appKeyFlagCache:appMapNew

2.增加log.error日志，让运维增加告警，能及时发现该问题

3.增加缓存查询功能，能有效查询缓存的内容
...
- [ ] 完校后台日志信息发送kafka兼容改造（待升级）
- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（20%）
- [ ] 中行宿主对接问题处理
- [ ] 处理kafka发送测试问题
...
- [ ] 运营平台广告个人屏蔽改造（升级中）
- [ ] 完校后台日志信息发送kafka兼容改造（升级中）
- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（50%）
- [ ] 中行宿主对接问题处理（因为中行不让直接嵌入B模块地址，所以学校做了一个nginx反向代理到B模块，但是B模块域名和学校域名不同导致，cookies无法写入。处理办法：放学校再代理开放平台的地址，在B模块将cookies写入域名改为学校地址，也就是B模块和开放平台都使用学校的域名地址访问）
- [ ] 处理kafka发送测试问题（已完成）
- [ ] 河南农大宿主开发（待处理）
...
- [ ] 运营平台广告个人屏蔽改造（升级中）
- [ ] 完校后台日志信息发送kafka兼容改造（升级中）
- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（50%）
- [ ] 中行宿主对接问题处理（因为中行不让直接嵌入B模块地址，所以学校做了一个nginx反向代理到B模块，但是B模块域名和学校域名不同导致，cookies无法写入。处理办法：放学校再代理开放平台的地址，在B模块将cookies写入域名改为学校地址，也就是B模块和开放平台都使用学校的域名地址访问）
- [ ] 处理四川交通职业技术学院外嵌迪科获取不到用户信息问题（迪科域名地址不通导致）
- [ ] 河南农大宿主开发（待处理）
...
- [ ] 河南农大宿主开发（80%）
- [ ] 短信平台计数计费规则开发（待处理，可能引入学校体系）
- [ ] 处理kafka日志数据不一致问题(处理中)
- [ ] 处理红河职教城门锁扫码功能问题(处理中)
- [ ] 处理赤峰工业职业技术学院农行掌银获取openid失败问题(处理中)

2022.07.13

- [ ] 开放平台完校客户端JSapi权限限制功能开发（10%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（公测中，待测试，增加回调地址返回完校token功能）
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [ ] B模块安全测试漏洞处理（待处理）
...
- [ ] B模块安全测试漏洞处理（待处理）
- [ ] 处理外嵌云陕工应用白屏问题（用错宿主了）
- [ ] 协助处理编程课堂活动应用对接问题（处理中）
- [ ] 处理—秦农充值非授权接口问题（处理中）
- [ ] 处理陵水雅居乐双语学校 充值支付通道对接问题（处理中）
- [ ] 处理温大CAS对接问题（处理中）
...
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [ ] B模块安全测试漏洞处理（待处理）
- [ ] 处理外嵌云陕工应用白屏问题（用错宿主了，改用cassso宿主有证书问题，处理中，需要在jdk中添加信任证书）
- [ ] 处理合肥万通微信H5支付标准宿主对接问题（宿主参数配置错误）
- [ ] 处理广告屏蔽生产不生效问题（处理中）
- [ ] 处理海南大学新生身份核验需求宿主对接问题（宿主参数配置错误）
...
- [ ] 开放平台完校客户端JSapi权限限制功能开发（20%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（公测中，待测试，增加回调地址返回完校token功能）
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
...
- [ ] 开放平台完校客户端JSapi权限限制功能开发（30%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（公测中，待测试，增加回调地址返回完校token功能）
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
...
遇到问题并不可怕，可怕的是你没有解决它的决心和勇气。

- [ ] 开放平台完校客户端JSapi权限限制功能开发（45%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（公测中，待测试，增加回调地址返回完校token功能）
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
...
- [ ] 开放平台完校客户端JSapi权限限制功能开发（60%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（公测中，待测试，增加回调地址返回完校token功能）
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [ ] 处理短信平台测试问题（已完成）
- [ ] e江南应用宿主对接开发（待处理）
- [ ] 处理—秦农应用充值对接非授权接口问题（处理中，接口需要加密，待开发）
- [ ] 协助处理南京工业大学企业号虚拟卡打开慢问题（处理中）
- [ ] B模块漏洞处理(待处理)
...
- [ ] 思政云应用宿主对接（待处理）
- [ ] cookies问题处理（已完成）
- [ ] 陕西中医院大学宿主画时序图、增加请求日志耗时、优化请求逻辑（待处理）
...
- [ ] 开放平台完校客户端JSapi权限限制功能开发（80%，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [ ] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [ ] 华为钱包宿主测试（公测中，待测试，增加回调地址返回完校token功能）
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
...
- [ ] 思政云应用宿主对接（待处理）
- [ ] cookies安全问题处理（已完成）
- [ ] 陕西中医院大学宿主画时序图、增加请求日志耗时、优化请求逻辑（已完成）
- [ ] 处理短信平台升级问题（缺少数据库连接池jar导致）
- [ ] 明天执行积分平台接口脚本（待处理）
...
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [x] e江南应用宿主对接开发（cas对接，不需要开发）
- [x] 思政云应用宿主对接（待处理，移交刘远、刘泉开发）
- [x] 执行积分平台接口脚本（已执行）
- [ ] 升级公测tomcat版本（已完成）
- [ ] 腾讯微校(卡)宿主（待开发，需要将flag、微卡accessToken、userId往后传）
...
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [x] e江南应用宿主对接开发（cas对接，不需要开发）
- [x] 思政云应用宿主对接（待处理，移交刘远、刘泉开发，下周1开始）
- [ ] 腾讯微校(卡)宿主（待开发，需要将flag、微卡accessToken、userId往后传）
- [ ] 秦龙银行对接支付非授权接口加密（待处理，预计下周4开始）
- [ ] B模块请求传参参数加密处理（待处理，给靳浩东提供数据）
...
- [ ] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [ ] e江南应用宿主对接开发（cas对接，不需要开发）
- [ ] 思政云应用宿主对接（移交刘远、刘泉开发，下周1开始）
- [ ] 腾讯微校(卡)宿主（60%，需要将flag、微卡accessToken、userId往后传）
- [ ] 秦龙银行对接支付非授权接口加密（待处理，预计下周4开始）
- [ ] B模块请求传参参数加密处理（50%，给靳浩东提供数据）

====================

2022.08.01

- [x] 开放平台完校客户端JSapi权限限制功能开发（公测中，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，7月完成第一版）
- [x] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [x] 华为钱包宿主测试（公测中，待测试，增加回调地址返回完校token功能）
- [x] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [x] 思政云应用宿主对接（移交刘远、刘泉开发，下周1开始）
- [x] 腾讯微校(卡)宿主（60%，需要将flag、微卡accessToken、userId往后传）
- [ ] 秦龙银行对接支付非授权接口加密（待处理，预计下周4开始）
- [x] B模块请求传参参数加密处理（50%，给靳浩东提供数据）
...
- [ ] B模块请求传参参数加密处理（公测中）
- [ ] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 处理郑州工商学院物联网水电系统微信H5缴电费嵌入腾讯微校问题（处理中）
- [ ] 处理湖南化工职业技术学院代扣应用嵌入工行手机银行问题（处理中）
- [ ] 处理泉州海洋职业学院自助充值对接问题（处理中）
- [x] 处理大连职业技术学院融合身份平台预约功能对接问题（已完成）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（待开发沟通）
- [ ] 处理企业微信新授权流程问题（待处理，预计下周2开始）
- [ ] 移动SIM卡标准宿主对接（处理中）
...
- [ ] 处理泉州海洋职业学院自助充值对接问题（处理中）
- [x] 处理大连职业技术学院融合身份平台预约功能对接问题（已完成）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（待开发沟通，预计下周4开始）
- [ ] 处理企业微信新授权流程问题（待处理，预计下周2开始）
- [ ] 移动SIM卡标准宿主对接（处理中）
...
- [ ] 处理企业微信新授权流程问题（待处理，预计下周2开始）
- [ ] 移动SIM卡标准宿主对接（处理中）
- [ ] 青海师范大学-请假+销假+审批外嵌问题处理（处理中，需要王回调地址传完校token）
- [ ] 
...
- [ ] 处理湖南化工职业技术学院代扣应用嵌入工行手机银行问题（处理中）
- [ ] 处理泉州海洋职业学院自助充值对接问题（处理中）
- [ ] 处理大连职业技术学院融合身份平台预约功能对接问题（已完成）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（待开发沟通，预计下周4开始）
- [ ] 处理企业微信新授权流程问题（待处理，预计下周2开始）
- [ ] 移动SIM卡标准宿主对接（处理中）
- [ ] 青海师范大学-请假+销假+审批外嵌问题处理（处理中，需要王回调地址传完校token）
- [ ] 处理徐州市委党校标准宿主对接问题（处理中）
- [ ] 处理银联开放平台接口对接问题（处理中）
...
- [x] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [x] 思政云应用宿主对接（移交刘远、刘泉开发，联调中）
- [x] 腾讯微校(卡)宿主（已升级）
- [ ] 秦龙银行对接支付非授权接口加密（开发中）
- [x] B模块请求传参参数加密处理（已升级）
- [x] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 处理郑州工商学院物联网水电系统微信H5缴电费嵌入腾讯微校问题（处理中）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（联调中，公测中）
- [ ] 处理企业微信新授权流程问题（待处理，预计下周2开始）
- [ ] 青海师范大学-请假+销假+审批外嵌问题处理（处理中，需要王回调地址传完校token）
- [ ] 处理徐州市委党校标准宿主对接问题（处理中）
- [ ] 处理银联开放平台接口对接问题（处理中）
...
- [ ] 处理徐州市委党校标准宿主对接问题（处理中）
- [ ] 处理银联开放平台接口对接问题（处理中）
- [ ] 处理芜湖职业移动端对接开放平台问题（处理中）
- [ ] 处理罗田实验中学工行E生活对接问题（处理中）
- [ ] 处理公测更换域名导致大部分应用无法使用问题（处理中）
- [ ] 处理福建水电缴费嵌入玩校问题（处理中）
...
- [x] 河南农大宿主开发（公测中）
- [ ] 中浦院联想应用宿主开发（待处理，需要向后传递加密字段）
- [x] 思政云应用宿主对接（移交刘远、刘泉开发，公测联调中）
- [x] 腾讯微校(卡)宿主（已升级）
- [ ] 秦龙银行对接支付非授权接口加密（开发中，70%）
- [x] B模块请求传参参数加密处理（已升级）
- [x] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 处理郑州工商学院物联网水电系统微信H5缴电费嵌入腾讯微校问题（处理中）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（联调中，公测中）
- [ ] 处理企业微信新授权流程问题（已上线，后期需要协助刘泉设计开发获取待开发应用secret流程）
- [ ] 青海师范大学-请假审批外嵌建融慧学app问题处理（公测中，宿主需要往回调地址传完校token）
- [ ] 处理罗田实验中学工行E生活对接问题（处理中）
- [ ] 处理公测更换域名导致大部分应用无法使用问题（处理中）
- [ ] 协助处理刘远新微信支付方式openid不一致问题（处理中）
...
- [x] cassso宿主增加ewpp获取支付信息开发（已升级，待测试）
...
- [ ] 处理郑州工商学院物联网水电系统微信H5缴电费嵌入腾讯微校问题（已处理）

- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（已升级）

- [ ] 企业微信新授权流程开发（50%， 协助刘泉设计开发获取待开发应用secret流程）
...
- [x] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（联调中，公测中）
- [ ] 企业微信新授权流程开发（公测中， 协助刘泉设计开发获取待开发应用secret流程）
- [ ] 科士达充电桩技术对接问题处理（处理中）
...
- [ ] 秦龙银行对接支付非授权接口加密（公测中）
- [x] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（线上联调中）
- [ ] 企业微信新授权流程开发（公测中， 协助刘泉设计开发获取待开发应用secret流程）
- [ ] 科士达充电桩技术对接问题处理（处理中）
- [ ] 协助刘远处理茅台学院迎新学生端缴费异常问题（处理中）
- [ ] 处理中南财经政法大学武汉学院外嵌到学校企业微信cas对接问题（处理中）
...
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（线上联调中）
- [ ] 企业微信新授权流程开发（公测中， 协助刘泉设计开发获取待开发应用secret流程）
- [ ] 处理ApacheCommonsConfiguration漏洞问题（该jar包可以直接删除，好像没用到）
- [ ] 协助李战立处理支付openid不一致问题（处理中）
- [ ] 协助王兰芳处理生产建行应用外嵌水电调试问题（处理中）
- [ ] 处理生产环境中财武汉学院外嵌水电应用对接问题（处理中）

=====================================================

2022.08.22

- [x] 开放平台完校客户端JSapi权限限制功能开发（公测中，①获取签名接口，②签名获取权限列表(签名和权限列表都有超时时间)，注意三方应用要有刷新签名的功能，沟通前端进度）
- [x] cassso宿主增加ewpp获取支付信息开发（公测中，待测试）
- [x] 华为钱包宿主测试（待升级）
- [ ] 中浦院联想应用宿主开发（待联调，之后加密传参给李毫华）
- [ ] 秦龙银行对接支付非授权接口加密（待升级）
- [ ] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 处理昆明卫生职业学院一卡通新宿主对接问题（处理中）
- [ ] 整理非服务商openid流转流程（已完成）
- [ ] 福州职业技术学院宿管系统对接开放平台问题处理（处理中）
- [ ] 协助企业微信新授权流程开发测试工作（测试中）
- [ ] 协助秦龙银行对接支付非授权接口加密测试工作（测试中）
- [ ] 青岛黄海学院将缴费平台对接开放平台问题处理（处理中）
...
- [ ] 福州职业技术学院宿管系统对接开放平台问题处理（处理中）
- [ ] 协助企业微信新授权流程开发测试工作（已上线）
- [ ] 协助秦龙银行对接支付非授权接口加密测试工作（已升级）
- [ ] 青岛黄海学院将缴费平台对接开放平台问题处理（处理中）
- [ ] 中信银行宿主开发（待提供有效开发文档）
- [ ] 工行应用对接（旧对接方式不再使用，待处理）
- [ ] 临商银行宿主开发（待处理）
- [ ] 处理开放平台03节点服务器启动失败问题（开放平台数据库vpc地址连接超时，但是改为经典地址可以正常启动）
...
- [ ] 工行应用对接（旧对接方式不再使用，60%）
- [ ] 临商银行宿主开发（待开发）
- [ ] 处理台州职业技术标卡交易明细评价抽奖增加开放平台详情接口字段（公测中）
- [ ] 处理滇西应用学校多应用分组绑卡问题（已处理）
...
- [ ] 处理秦淮科技高中访问完校开放平台接口不通问题（一卡通报错）

- [ ] 处理西山学校-标卡嵌入农行访问开放平台接口不通问题（未提供非授权接口配置）

- [ ] 处理无锡南洋职业技术学院充值对接访问开放平台接口不通问题（未提供非授权接口配置）
...
- [ ] 排查B模块响应时间过长问题（部分学校宿主用户接口访问超时导致）

- [ ] 处理南工大个别用户打开虚拟卡是空白页问题（一卡通接口返回用户虚拟卡被禁导致）
...
- [ ] 临商银行宿主开发（公测中）
- [ ] 处理台州职业技术标卡交易明细评价抽奖增加开放平台详情接口字段（公测中）
- [ ] 建融汇学自动绑卡宿主开发（公测中，配合宿主测试上线）
- [ ] 系统表单js限制问题处理（公测中）
- [ ] 中信银行宿主开发（待联调）
- [ ] 西安外国语大学学校应用宿主对接（待开发）
- [ ] 处理B模块迁移VPC环境农行接口报错问题（已处理）
- [ ] 宿主客户映射管理增加平台标记查询条件（待开发）
...
- [ ] 中信银行宿主开发（待联调）
- [ ] 处理B模块迁移VPC环境农行接口报错问题（已处理）
- [ ] 宿主客户映射管理增加平台标记查询条件（公测中）
- [ ] 新增宿主数据复制功能（公测中）
- [ ] 处理山西传媒学院企业微信问题（处理中）
- [ ] 协助处理南阳理工学院外嵌标卡到树维超级app打开慢问题（处理中）
- [ ] 处理广州松田职业学院移动SIM卡生产配置问题（处理中）
...
- [ ] 系统表单js限制问题处理（公测中）
- [ ] 中信银行宿主开发（待联调）
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待处理）
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 处理浙师大保卫处访客需求外嵌访客系统问题（处理中）
...
- [ ] 企业微信新授权流程二次开发（刘泉开发中，60%）
- [ ] 处理台州职业技术标卡交易明细评价抽奖增加开放平台详情接口字段（公测中）
- [ ] 系统表单js限制问题处理，新增宿主复制和查询条件（公测中）
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待处理）
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 处理浙师大保卫处访客需求外嵌访客系统问题（处理中，需要将三方信息传给业务系统，待开发处理）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（方案待待确认，1是通过url传递，需要加密？2是存在B模块关系表中提供开放平台外部接口访问能力，但是要加索引）
...
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 处理浙师大保卫处访客需求外嵌访客系统问题（处理中，需要将三方信息传给业务系统，待开发处理）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（是存在B模块关系表中提供开放平台外部接口访问能力，但是要加索引）
- [ ] 协助处理上海外国语学校外嵌报到统计系统访问异常问题（报到统计系统入口地址访问不通导致）
- [ ] 处理焦作新材料学院餐厅对接开发开放平台接口对接问题（处理中）
...
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待处理）
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（30%，是存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 协助处理上海外国语学校外嵌报到统计系统访问异常问题（报到统计系统入口地址访问不通导致）
- [ ] 海南医学院学校宿主开发（待处理）
...
- [ ] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 企业微信新授权流程二次开发（刘泉开发中，60%）
- [ ] 处理台州职业技术标卡交易明细评价抽奖增加开放平台详情接口字段（已上线）
- [ ] 系统表单js限制问题处理，新增宿主复制和查询条件（已上线）
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待处理）
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（30%，是存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 协助处理上海外国语学校外嵌报到统计系统访问异常问题（报到统计系统入口地址访问不通导致）
- [ ] 协助排查青海师范大学外嵌会话失效问题（调用了私有接口，但是没有访问权限）
- [ ] 协助排查河大民生学院访问开放平台接口异常问题（前置或一卡通配置异常）
- [ ] 海南医学院学校宿主开发（待处理）
- [ ] 讯飞教育云平台宿主开发（待处理）
...
- [ ] 中浦院联想应用宿主开发（公测中，和在线报修调试中）
- [ ] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 企业微信新授权流程二次开发（刘泉开发完成，待测试）
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待处理）
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（30%，是存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 海南医学院学校宿主开发（待处理）
- [ ] 讯飞教育云平台宿主开发（待处理）
- [ ] 成都铁路卫生学校标准宿主外嵌标卡问题处理（处理中）
- [ ] 处理重庆文理学院融合平台外嵌宿主问题（处理中）
- [ ] 协助处理浙师大外嵌访客无一卡通环境问题处理（已完成）
...
- [ ] 海南医学院学校宿主开发（待处理）
- [ ] 讯飞教育云平台宿主开发（待处理）
- [x] 处理焦作新材料学院对接口开放平台接口问题处理（处理中）
- [ ] 处理临商银行生产配置问题（处理中）
- [ ] 处理张育坤银联签约外嵌树维应用对接问题（处理中）
- [ ] 处理铜梁一中外嵌第三方电表系统到农行掌银中问题处理（处理中）
- [ ] 
...
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待处理）
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（待测试，是存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 海南医学院学校宿主开发（70%）
- [ ] 讯飞教育云平台宿主开发（待处理）
- [ ]  浙江水利水电学院学校宿主开发（待处理）
- [x] 处理焦作新材料学院对接口开放平台接口问题处理（处理中）
...
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待处理）
- [ ] 陵水国际教育园区对接开放平台问题处理（处理中）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（待测试，存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 海南医学院学校宿主开发（待联调）
- [ ] 讯飞教育云平台宿主开发（待处理）
- [ ] 浙江水利水电学院学校宿主开发（50%）
- [x] 处理焦作新材料学院对接口开放平台接口问题处理（处理中）
...
- [ ] 讯飞教育云平台宿主开发（待处理）
- [ ] 兴业银行单账户覆盖绑卡宿主开发（待处理）
- [x] 处理焦作新材料学院对接口开放平台接口问题处理（已完成）
- [ ] 处理临商银行生产配置问题（已完成）
- [ ] 德育轻应用对接开发问题处理（已完成）
- [ ] 处理铜梁一中外嵌第三方电表系统到农行掌银中问题处理（已处理，切换为通用门户宿主进行自动绑卡）
- [ ] 处理中国刑事警察学院CAS对接问题（已处理）
...
- [ ] 讯飞教育云平台宿主开发（10%）
- [ ] 兴业银行游客登录模式宿主开发（待处理）
- [ ] 协助处理邮储APP外嵌一体化门户双击返回异常问题（已处理，通过直接跳过light页面解决）
- [ ] 协助排查广西建行某用户非授权接口数据不正常问题（处理中，该用户账号被注销重录过，开放平台缓存数据未更新）
...
- [ ] 山西机电职业技术学院宿主对接（待处理）
- [ ] 四川电影电视学院宿主对接（待处理）
- [ ] 紫金学院钉钉对接外嵌应用无法正常使用问题处理（处理中）
- [ ] 宿主客户映射管理中宿主客户编码字段需要保证唯一改造（待处理）
...
- [ ] 宿主客户映射管理中宿主客户编码字段需要保证唯一改造（待处理）
- [ ] B模块开放平台服务迁移云服务器（待处理）
- [ ] 处理海南医学院完美校园访问人脸采集白屏问题（人脸采集应用挂了）
...
- [ ] 宿主客户映射管理中宿主客户编码字段需要保证唯一改造（待处理）
- [ ] B模块开放平台服务迁移云服务器（待处理）
- [ ] 处理焦作新材料学院餐厅对接开放平台接口问题（处理中）
- [ ] 处理桂阳职业教育技术学院外嵌建行E码通问题（宿主配置错误）
...
- [ ] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 企业微信新授权流程二次开发（刘泉开发完成，待测试）
- [ ] 处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题（待升级）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（待测试，存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 讯飞教育云平台宿主开发（公测中）
- [ ] 兴业银行游客登录模式宿主开发（公测中）
- [ ] 山西机电职业技术学院宿主对接（50%）
- [ ] 四川电影电视学院宿主对接（待处理）
- [ ] 宿主客户映射管理中宿主客户编码字段需要保证唯一改造（公测中）
- [ ] B模块开放平台服务迁移云服务器（待处理）
- [ ] 处理焦作新材料学院餐厅对接开放平台接口问题（处理中）
...
- [ ] B模块开放平台服务迁移云服务器（待处理）
- [ ] 处理焦作新材料学院餐厅对接开放平台接口问题（处理中）
- [ ] 处理潍坊科技学院外嵌环境迁移调试开放平台接口问题（处理中）
- [ ] 处理一卡通账号被注销的完校账号无法正常进入标卡问题（处理中）
- [ ] 安徽中医药大学增加开放平台查询一卡通接口（待提供文档）
...
- [ ] 处理焦作新材料学院餐厅对接开放平台接口问题（处理中）
- [ ] 处理潍坊科技学院外嵌环境迁移调试开放平台接口问题（处理中）
- [ ] 处理一卡通账号被注销的完校账号无法正常进入标卡问题（50%，待提供提示页面）
- [ ] 安徽中医药大学增加开放平台查询一卡通接口（待提供文档）
...
- [ ] 安徽中医药大学增加开放平台查询一卡通接口（待提供文档）
- [ ] 建融汇学宿主外嵌请假审批传递手机号改造（待处理）
- [ ] 开放平台返回页面改友好html页面提示（待处理）
- [ ] 成都铁路卫生学校标准对接问题处理（处理中）
...
- [ ] 宿主客户映射管理中宿主客户编码字段需要保证唯一改造（公测中）
- [ ] B模块开放平台服务迁移云服务器（已完成）
- [ ] 处理一卡通账号被注销的完校账号无法正常进入标卡问题（已升级）
- [ ] 安徽中医药大学增加开放平台查询一卡通接口（50%）
- [ ] 建融汇学宿主外嵌请假审批传递手机号改造（待处理）
- [ ] 开放平台返回页面改友好html页面提示（待处理）
- [ ] 中行宿主国密改造（待处理）
- [ ] 开放平台新增退款接口(支持部分退款、跨天退款，待处理)
...
- [ ] 开放平台返回页面改友好html页面提示（待处理）
- [ ] 中行宿主国密改造（待处理）
- [ ] 开放平台新增退款接口(支持部分退款、跨天退款，80%)
- [ ] 处理一卡通账号不存在解绑问题（待升级）
- [ ] 处理广西经济职业学院标准宿主对接（处理中）
...
- [ ] 开放平台返回页面改友好html页面提示（待处理）
- [ ] 中行宿主国密改造（待处理）
- [ ] 开放平台新增退款接口(支持部分退款、跨天退款，公测中，待调试)
- [ ] 处理一卡通账号不存在解绑问题（待升级）
- [ ] 处理广西经济职业学院标准宿主对接（处理中）
...
- [ ] 中行宿主国密改造（待处理）
- [ ] 开放平台新增退款接口(支持部分退款、跨天退款，公测中，待调试)
- [ ] 处理开放平台非授权接口缓存问题（50%，MQ订阅解决）
- [ ] 处理一卡通账号不存在解绑问题（待升级）
- [ ] 处理广西经济职业学院标准宿主对接（处理中）
- [ ] 省幼师oa与开放平台对接问题处理（处理中）
- [ ] 考勤系统对接开放平台问题处理（处理中）
- [ ] 处理遵义医药高等专科学校树维外嵌标卡报错问题（树维多传了无法解析的参数）
...
- [ ] 省幼师oa与开放平台对接问题处理（处理中）
- [ ] 考勤系统对接开放平台问题处理（处理中）
- [ ] 校园卡外嵌上海建行通用平台问题处理（三方传递参数异常导致）
- [ ] 惠安亮亮中学宇视人脸门禁与一卡通对接开放平台接口问题处理（处理中）
...
- [ ] 安徽中医药大学增加开放平台查询一卡通接口（已完成）
- [ ] 建融汇学宿主外嵌请假审批传递手机号改造（待处理）
- [ ] 开放平台返回页面改友好html页面提示（50%）
- [ ] 开放平台字典参数同步改造（70%）
- [ ] 中行宿主国密改造（调试中）
- [ ] 验证码业务安全一卡通接口对接（公测中）
- [ ] B模块增加标卡快速入口生成功能（待开发）
- [ ] 处理邮储APP标卡快速跳转传参问题（处理中）
...
- [ ] B模块增加标卡快速入口生成功能（待开发）
- [ ] 讯飞教育云平台移动端宿主开发（待开发）
- [ ] 处理青海师范外嵌访客到树维入口异常问题（树维用户token传参重复导致）
...
- [ ] 讯飞教育云平台宿主开发（20%，移动端宿主开发中）

- [ ] 安徽中医药大学增加开放平台查询一卡通接口（已完成）

- [ ] 建融汇学宿主外嵌请假审批传递手机号改造（待处理）

- [ ] 开放平台返回页面改友好html页面提示（80%）
...
- [ ] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（暂停中，王兰芳反馈需求有变化，存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 讯飞教育云平台宿主开发（30%，移动端宿主开发中）
- [ ] 安徽中医药大学增加开放平台查询一卡通接口（已完成）
- [ ] 建融汇学宿主外嵌请假审批传递手机号改造（待处理）
- [ ] 开放平台返回页面改友好html页面提示（90%）
- [ ] 开放平台字典参数同步改造（ 待升级）
- [ ] 验证码业务安全一卡通接口对接（公测中）
- [ ] B模块增加标卡快速入口生成功能（待开发）
...
- [ ] B模块增加标卡快速入口生成功能（待开发）
- [ ] 验证码业务安全一卡通接口对接（联调中）
- [ ] 协助处理重庆广益中学工银E钱包充值对接问题（处理中）
- [ ] 处理企业微信接口收费限制使用问题（处理中，企业微信OAuth授权接口调用要收费了）
- [ ] 协助张春枫处理山东建行充值接口问题（处理中）
- [ ] 处理杨会东公测环境系统漏洞问题（已完成）
...
- [ ] 河南工业职业技术学院疫情防控系统内的访客模块树维对接（需要增加待办数目接口通过开放平台向三方暴露，待开发）
- [ ] 全部宿主支持给业务系统传递三方用户信息功能开发（暂停中，王兰芳反馈需求有变化，存在B模块关系表中提供开放平台外部接口访问能力，但要加索引）
- [ ] 讯飞教育云平台宿主开发（60%，移动端宿主开发中） 
- [ ] 安徽中医药大学增加开放平台查询一卡通接口（已完成）
- [ ] 建融汇学宿主外嵌请假审批传递手机号改造（待处理）
- [ ] 开放平台返回页面改友好html页面提示（公测中）
- [ ] 开放平台字典参数同步改造（ 待测试）
- [ ] B模块增加标卡快速入口生成功能（待开发）
- [ ] 验证码业务安全一卡通接口对接（联调中）
- [ ] 协助处理重庆广益中学工银E钱包充值对接问题（处理中）
...
- [ ] B模块增加标卡快速入口生成功能（待开发）
- [ ] 验证码业务安全一卡通接口对接（联调中）
- [ ] 北大点餐对接开放平台接口问题处理（处理中）
- [ ] 江西省广丰贞白中学外嵌银联云闪付app问题处理（处理中）
- [ ] 西南大学对接开放平台接口问题处理（处理中）
- [ ] 处理企业微信接口收费限制使用问题（处理中，下周1需要整理出企业微信自建应用创建流程、反向代理测试方案）
- [ ] 湖南涉外经济学院外嵌超星APP问题处理（处理中）
- [ ] 山东大学限制部分类型卡用户绑卡功能开发（待开发）
...
- [ ] 江西省广丰贞白中学外嵌银联云闪付app问题处理（处理中）
- [ ] 西南大学对接开放平台接口问题处理（处理中）
- [ ] 处理企业微信接口收费限制使用问题（100%）
- [ ] 处理刘泉建行开放平台接口调试问题（处理中）
- [ ] 山东大学限制部分类型卡用户绑卡功能开发（待开发）
...
- [ ] 验证码业务安全一卡通接口对接（待升级）
- [ ] 江西省广丰贞白中学外嵌银联云闪付app问题处理（处理中）
- [ ] 企业微信外嵌应用代理访问预研及相关问题处理（100%）
- [ ] 山东大学限制部分类型卡用户绑卡功能开发（5%）
...
- [ ] 验证码业务安全一卡通接口对接（待升级）
- [ ] 山东大学限制部分类型卡用户绑卡功能开发（100%）
- [ ] 开放平台请求一卡通接口做熔断降级处理（5%）
- [ ] 协助刘泉处理亮亮中学充值获取不到openid问题（处理中）
- [ ] 协助许娇测试山东大学限制部分类型卡用户绑卡功能（处理中）
...
- [ ] 验证码业务安全一卡通接口对接（待升级）
- [ ] 山东大学限制部分类型卡用户绑卡功能开发（100%）
- [ ] 开放平台请求一卡通接口做熔断降级处理（10%）
- [ ] 协助许娇测试山东大学限制部分类型卡用户绑卡功能（已完成）
- [ ] 处理黄淮学院外嵌应用树维宿主快速跳转改造（待处理）
- [ ] 协助谢金宝处理南洋理工外嵌打卡到树维应用问题（处理中）
...
- [ ] B模块增加标卡快速入口生成功能（公测中）
- [ ] 验证码业务安全一卡通接口对接（待升级）
- [ ] 山东大学限制部分类型卡用户绑卡功能开发（待升级）
- [ ] 开放平台请求一卡通接口做熔断降级处理（15%）
- [ ] 处理黄淮学院外嵌应用树维宿主快速跳转改造（50%）
- [ ] 
...
- [ ] B模块增加标卡快速入口生成功能（公测中）
- [ ] 验证码业务安全一卡通接口对接（待升级）
- [ ] 山东大学限制部分类型卡用户绑卡功能开发（已升级）
- [ ] 开放平台请求一卡通接口做熔断降级处理（15%）
- [ ] 处理黄淮学院外嵌应用树维宿主快速跳转改造（100%）
- [ ] 处理开放平台新建表无法添加索引问题（索引字段长度被字符集锁限制导致）
- [ ] 处理茂名职业技术外嵌企业微信走代理访问模式问题（处理中）
- [ ] 宁夏新商务技工学校开发一卡通定制接口（待开发）
...
- [ ] B模块增加标卡快速入口生成功能（公测中）
- [ ] 开放平台请求一卡通接口做熔断降级处理（15%）
- [ ] 宁夏新商务技工学校开发一卡通定制接口（30%）
- [ ] 处理茂名职业技术外嵌企业微信走代理访问模式问题（处理中）
- [ ] 处理黄淮学院外嵌应用树维宿主快速跳转改造（调试中）
...
- [ ] B模块增加标卡快速入口生成功能（公测中）
- [ ] 开放平台请求一卡通接口做熔断降级处理（15%）
- [ ] 宁夏新商务技工学校开发一卡通定制接口（60%）
- [ ] 处理茂名职业技术外嵌企业微信走代理访问模式问题（处理中,企业可信ip无法设置成功，方案失败）
- [ ] 处理黄淮学院外嵌应用树维宿主快速跳转改造（调试中）
...
- [ ] 开放平台返回页面改友好html页面提示（公测中，已升级一部分）
- [ ] B模块增加标卡快速入口生成功能（测试中）
- [ ] 开放平台请求一卡通接口做熔断降级处理（20%）
- [ ] 宁夏新商务技工学校开发一卡通定制接口（待联调）
- [ ] 企业微信外嵌应用相关问题处理（处理中）
- [ ] 处理黄淮学院外嵌应用树维宿主快速跳转改造（调试中）
- [ ] 处理日志平台漏洞问题（待升级）
...
- [ ] 开放平台返回页面改友好html页面提示（公测中，已升级一部分）
- [ ] B模块增加标卡快速入口生成功能（测试中）
- [ ] 开放平台请求一卡通接口做熔断降级处理（30%）
- [ ] 宁夏新商务技工学校开发一卡通定制接口（联调中）
- [ ] 处理黄淮学院外嵌应用树维宿主快速跳转改造（待升级）
- [ ] 处理日志平台漏洞问题（待升级）
- [ ] 企业微信外嵌应用相关问题处理（已处理，通过新开服务器更换企业出口ip，成功的走通代理方案）
- [ ] 开放平台新增台州职业-餐厅消费S01046接口（50%）
- [ ] 功能管理平台新版皮肤功能开发（待开发）
...
09-13~09-19 B模块直接传递三方信息给访客系统流程开发（该需求需要改动所有宿主，暂时只做cassso宿主的）

09-20~09-22 海南医学院学校宿主开发

09-22~09-23 浙江水利水电学院学校宿主对接开发

09-26~09-29 讯飞教育云平台宿主开发

09-30~10-11 兴业银行游客登录模式宿主开发

10-11~10-12 宿主客户映射管理中宿主客户编码字段需要保证唯一改造、优化查询逻辑、处理BSACS_ACCOUNT_USE_LOG表无法存储emoji问题

10-12~10-14 山西机电职业技术学院宿主对接

10-17~10-18 安徽中医药大学增加开放平台查询一卡通接口开发

10-19~10-21 四川电影电视学院宿主对接

10-21~10-25 开放平台新增退款接口

10-24~10-26 非授权接口缓存问题处理

10-27~11-01 中行宿主国密改造

11-01~11-02 验证码业务安全一卡通接口对接

11-03~11-11 开放平台字典参数同步改造及入口请求页面优化

11-14~11-18（40h） 讯飞移动端宿主对接

11-18~11-23（32h）  企业微信代理访问外嵌预研

11-23-11-28（32h）  山东大学限制部分类型卡用户绑卡功能开发

11-29-11-30（16h） 标卡快速入口地址生成功能开发



12-01-12- 开放平台请求一卡通接口做熔断降级处理

12-05-12-06（16h）处理黄淮学院外嵌应用树维宿主快速跳转改造

12-07-12-09（24h）~~开放平台开发宁夏新商务技工学校一卡通定制接口~~

12-12-12-13（16h）企业微信外嵌代理方案验证

12-14-12-14（8h）开放平台新增台州职业-餐厅消费S01046接口

12-21-12-23（24h）开放平台公测环境迁移

12-22-12-29（40h）功能管理平台新版皮肤功能开发
...
[mysql5.1升级到mysql5.7（使用yum源）-朱双印博客 (zsythink.net)](https://www.zsythink.net/archives/2345)

[yum方式的mysql升级版本步骤_叶落繁谣的博客-CSDN博客_yum 升级mysql](https://blog.csdn.net/qq_45391176/article/details/125162146)

[linux 升级 MySQL5.7.32 mysql升级（yum方式） - 走看看 (zoukankan.com)](http://t.zoukankan.com/laod-p-14278429.html)

[MySQL-Linux升级MySQL - 我是一粒沙 - 博客园 (cnblogs.com)](https://www.cnblogs.com/tesila/p/15228093.html)
...
eacrd_fast_use: 标卡快速入口、线程绑卡flagCookie、黄淮学院宿主改造

gyfCustomToken：自定义token名称(龚一凡)
...
secret_interface_use：增加一卡通交易安全密码接口

S01046_use：增加台州职业-餐厅消费S01046接口

ningxiaEcard_use：增加学校定制一卡通接口

sentinel_use：增加sentinel规则能力
